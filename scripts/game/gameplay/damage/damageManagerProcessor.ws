...
class W3DamageManagerProcessor extends CObject 
{
	...
	private function ProcessActionDamage() : bool
	{
		...
		// KOTW ARMORS - Begin
		kotwArmors().SetInfusionVariables(action, attackAction);
		kotwArmors().DealInfusionDamage(action);
		
		kotwArmors().IncreaseDimeritiumCharge(action);
		kotwArmors().DischargeArmor(action);
		// KOTW ARMORS - End
